...
public class VelocityOnlyFileSerializer implements IPathSerializer {
	...
	/**
	 * Format: Each segment is in the format: t leftVelocity rightVelocity
	 * 
	 * @param path
	...
	public String serialize(Path path) {
		...
		for (int i = 0; i < path.getLeftWheelTrajectory().getNumSegments(); i++) {
			...
			builder.append(timeStamp).append("\t").append(leftVelocity).append("\t").append(rightVelocity).append("\n");
		}
